// 函数名称: sub_582320
// 虚拟地址: 0x582320
// 源二进制: E:/torrent/Cursor/Race/steamapps/common/Race for the Galaxy/Race.exe.bndb

LRESULT __fastcallsub_582320(WPARAM arg1)
{
    // 第一条实际指令: LRESULT lParam = SendMessageA(*(data_26a6554 + 0x18), 0x199, arg1, 0)
    LRESULT lParam = SendMessageA(*(data_26a6554 + 0x18), 0x199, arg1, 0)
    
    if (lParam != 0xffffffff && lParam != 0 && *(lParam + 0x14) != 0x63)
        LRESULT wParam = SendMessageA(*(data_26a6554 + 0x14), 0x18f, 0, lParam)
        
        if (wParam != 0xffffffff)
            LRESULT result = SendMessageA(*(data_26a6554 + 0x14), 0x186, wParam, 0)
            HWND ecx_1 = *(data_26a6554 + 0x14)
            int16_t eax_3 = GetDlgCtrlID(ecx_1)
            SendMessageA(GetParent(*(data_26a6554 + 0x14)), 0x111, zx.d(eax_3) | 0x10000, ecx_1)
            
            if (*(lParam + 0x14) == 8)
                *(lParam + 8) = &data_8954cc
            
            RedrawWindow(*(data_26a6554 + 0x14), nullptr, nullptr, 
                RDW_INVALIDATE | RDW_ERASE | RDW_UPDATENOW)
            return result
    
    return 0xffffffff
}
